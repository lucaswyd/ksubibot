...
// Serve the `public` folder for static assets (styles.css, etc.)
...
// Existing fnbr logic (same as your provided code)
(async () => {
    let auth;
    ...
    client.on("ready", async () => {
        ...
        if (client.friend) {
            client.on("friend:request", async (request) => {
                try {
                    ...
                    const senderName =
                        request.sender?._displayName ||
                        request.sender?.name ||
                        "Unknown Sender";
                    console.log(
                        `Automatically accepted a friend request from: ${senderName}`,
                    );
                } catch (error) {
        ...
        client.on("party:invite", async (invitation) => {
            ...
            try {
                ...
                const senderName =
                    invitation.sender?._displayName ||
                    invitation.sender?.name ||
                    "Unknown Sender";
                console.log(
                    `Automatically accepted a party invitation from: ${senderName}`,
                );
                ...
                setTimeout(async () => {
                    try {
                        ...
                        console.error(
                            "Failed to play the Slalom Style emote:",
                            error,
                        );
                    }
                }, 2000); // 2-second delay
